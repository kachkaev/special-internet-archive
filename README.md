...
1.  Создайте файл `/path/to/data/collections/COLLECTION_NAME/url-inbox.yml`.
    Это можно сделать вручную или скриптом:

    ```sh
    ## 🚧 🚧 🚧 пока не работает 🚧 🚧 🚧
    yarn exe scripts/web-pages/ensure-url-inbox-file.script.ts
    ```

1.  Накидайте в этот файл ссылки, которые хотите зарегистрировать.
    Пример:
...
1.  Запустите скрипт для регистрации всех этих ссылок из файла `url-inbox.yml`:
...
1.  ![][опционально]  
    Запустите скрипт, который удалит все зарегистрированные ссылки из файла `url-inbox.yml`.
    Это поможет отфильтровать ссылки, которые не получилось добавить.
...
1.  Получите список снимков для веб-страниц из нашей коллекции.
    ...
    Повторный запуск команды пропустит веб-страницы, которые проверяли до 10 минут назад.
...
1.  Обновите локальный список снимков для веб-страниц из нашей коллекции.
...
1.  TODO
    ...
    ```sh
    ## 🚧 🚧 🚧 пока не работает 🚧 🚧 🚧
    yarn exe scripts/web-pages/snapshots/generate-summaries.script.ts
    ```
...
### Добавление новых веб-страниц в коллекцию по аннотациям
