...
Снимки бывают локальными (хранятся в коллекции) и внешними (хранятся на сторонних серверах).

Локальные снимки более гибкие и содержательные, потому что мы сами управляем инструментами для их сбора.
Внешние снимки содержат меньше информации, но считаются более надёжными.
Чисто в теории локальные снимки могут быть подделкой, поэтому синхронизация с международно признанным интернет-архивом — это что-то вроде получения нотариально заверенной копии документа.
Даже если весь наш архив с данными уничтожить, внешние снимки веб-страниц всё равно останутся.

**Аннотация веб-страницы** — это дополнительные данные, которые были добавлены в процессе архивации.
Пример аннотаций — метки.
Они помогают собирать, структурировать и анализировать архивные данные.
Аннотации составляются по снимкам веб-страниц, при этом хранятся отдельно.
...
Основные форматы файлов — [JSON](https://ru.wikipedia.org/wiki/JSON) и [ZIP](https://ru.wikipedia.org/wiki/ZIP).

Для синхронизации собранных данных мы используем [гит](https://ru.wikipedia.org/wiki/Git) (систему контроля версий).
...
🗃 папка с архивом одной веб-страницы (адреса в сети)  
📂 любая другая папка  
🏛 файл с первичными данными (включен в коллекцию)  
⏳ временный файл (не включён в коллекцию)  
...
Папка для каждой веб-страницы (адреса в сети) выглядит так:
...
🗃 path/to/web-page/
  ...
  📂 snapshots/
    ...
    🏛 2022-04-05-124312z-playwright.zip
    ⏳ 2022-04-05-124312z-playwright.zip.summary.json
...
1.  Откройте файл `/path/to/tooling/.env.local` как текстовый и укажите путь к коллекции архива.
    ...
    Части `/path/to` и `COLLECTION_NAME` надо заменить.
...
Чтобы автоматизировать архивацию постов ВК, нам нужно «прочитать» ленты пабликов и найти в них ссылки типа `https://vk.com/wall-123-456`.
Сначала мы разбираем известные нам снапшоты и извлекаем сводку по каждому из них.
Так как у каждой веб-страницы может быть по несколько снапшотов, мы комбинируем полученные сводки отдельным скриптом.

1.  Извлеките сводки из снапшотов Playwright:
...
1.  Извлеките сводки из снапшотов Wayback Machine:
    ...
    Этот скрипт скачает известные нам снапшоты из `web.archive.org` и создаст файл `[web-page-dir]/snapshots/*-wayback-machine.zip.summary.json` для каждого файла `[web-page-dir]/snapshots/*-wayback-machine.zip`.
