...
> Архитектура этого репозитория позволяет работать с другими источниками веб-страниц в будущем.
...
Мы не используем базы данных или закрытые форматы файлов, чтобы минимизировать зависимость архива от доступных технологий для обработки данных.
...
Все файлы и папки опциональные: если вы их не видите в своей коллекции, всё в порядке.
...
Файл `⏳ url-inbox.txt` нужен [для регистрации веб-страниц](#регистрация-веб-страниц-списком).

Папка веб-страницы (адреса в интернете) выглядят так:
...
В файле `🏛 web-page.json` содержится адрес веб-страницы, списки известных снимков и аннотация.
...
Сводки [извлекаются из снимков скриптами](#извлечение-сводок-из-снимков) и содержат структурированные копии веб-страниц (например, список постов в ленте).
Файлы сводок считаются временными, потому что в них есть только данные внутри снимков.
Файл `⏳ snapshot-summary-combination.json` тоже генерируется локально и содержит комбинацию сводок (например, объединённый список постов в ленте сообщества).

Временные координаты в названиях файлов (`2022-04-05-124312z`) и внутри документов (`"2022-04-05T12:43:12Z"`) используют часовой пояс [UTC](https://ru.wikipedia.org/wiki/%D0%92%D1%81%D0%B5%D0%BC%D0%B8%D1%80%D0%BD%D0%BE%D0%B5_%D0%BA%D0%BE%D0%BE%D1%80%D0%B4%D0%B8%D0%BD%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D0%BE%D0%B5_%D0%B2%D1%80%D0%B5%D0%BC%D1%8F) (MSK-03).
...
Вложенных папок веб-страниц не бывает:
...
1.  Клонируйте созданную для вас ветку в папку `[project-path]/data/collections/[collection-id]`.
    ...
    `[collection-id]` — это название коллекции (например, `region-pnz` или `topic-xyz`).
...
Ссылки на посты регистрировать не обязательно, так как они автоматически извлекаются из лент сообществ.
...
1.  Извлеките сводки из снимков Wayback Machine:
    ...
    Этот скрипт скачает известные нам снимки из `web.archive.org` в `⏳ [web-page-path]/snapshots/*-wayback-machine.zip` и создаст файл `⏳ [web-page-path]/snapshots/*-wayback-machine.zip.summary.json` для каждого снимка.
...
1.  Скомбинируйте сводки:
    ...
    Этот скрипт прочитает файлы `⏳ [web-page-path]/snapshots/*.summary.json` и создаст файл `⏳ [web-page-path]/snapshot-summary-combination.json`.
