...
Архив содержит _снимки_ и _аннотации_ публичных веб-страниц (адресов в интернете).
...
Снимки бывают локальными (созданы волонтёром на своём компьютере) и внешними (созданы сторонними сервисами).
...
Они помогают собирать, структурировать и анализировать архивные данные.
Аннотации в основном составляются по снимкам веб-страниц, при этом всегда хранятся отдельно от них.

> ℹ️  
> На данном этапе архив фокусируется только на сообществах ВК: лентах и постах с комментариями.
> Архитектура позволяет работать с другими источниками страниц в будущем.
...
```txt
🗄 коллекция 1      🗄 коллекция 2      ...       🗄 коллекция N
↕                   ↕                             ↕
👤                  👤👤👤                         👤👤
```

Коллекция архива имеет формат обычной папки с файлами.
Мы не используем базы данных или закрытые форматы файлов, чтобы архив минимально зависел от доступности технологий.
Основные форматы файлов в коллекции — [JSON](https://ru.wikipedia.org/wiki/JSON) и [ZIP](https://ru.wikipedia.org/wiki/ZIP).
...
Система контроля версий не является обязательным компонентом для потребителей архива.
...
Папка `📂 web-pages` хранит данные по всем веб-страницам коллекции.
Структура этой папки определяется форматом интернет-адресов.
Например, `https://vk.com/wall-54321_111` соответствует папке `vk/posts/-54321/111`.

Папка `📂 snapshot-queues` используется при [создании и обновлении снимков](#создание-и-обновление-снимков).

Папка для каждой веб-страницы (адреса в интернете) выглядит так:
...
🗃 some/path/to/web-page/
  ...
  📂 snapshots/
    ...
    ⏳ 2022-04-07-071231z-some-other-external-generator.zip
    ⏳ 2022-04-07-071231z-some-other-external-generator.zip.summary.json
...
В файле `🏛 web-page.json` содержится адрес страницы, списки известных снимков и аннотации.

Папка `📂 snapshots` хранит все снимки конкретной веб-страницы.
Снимки, сгенерированные локально, становятся частью коллекции (🏛).
Снимки, сделанные при помощи внешних сервисов, считаются временными файлами (⏳).
Внешние снимки нет смысла хранить в коллекции, так их можно заново скачать.

<!-- ↑ Возможно, это правило будет пересмотрено. -->

Рядом со снимком страницы находится сводка по этому снимку (`⏳ *.summary.json`).
Сводки [извлекаются из снимков скриптами](#извлечение-сводок-из-снимков) и хранят структурированные копии страниц (например, список постов в ленте).
Файлы сводок считаются временными, потому что в них не содержится данных, которые бы отсутствовали в самих снимках.
Файл `⏳ snapshot-summary-combination.json` тоже генерируется локально и содержит комбинацию сводок (например, объединённый список постов в ленте).

Время в названиях файлов (`2022-04-05-124312z`) и внутри документов (`"2022-04-05T12:43:12Z"`) используют часовой пояс [UTC](https://ru.wikipedia.org/wiki/%D0%92%D1%81%D0%B5%D0%BC%D0%B8%D1%80%D0%BD%D0%BE%D0%B5_%D0%BA%D0%BE%D0%BE%D1%80%D0%B4%D0%B8%D0%BD%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D0%BE%D0%B5_%D0%B2%D1%80%D0%B5%D0%BC%D1%8F) (`MSK-03`).
[Согласно ISO 8601](<https://en.wikipedia.org/wiki/ISO_8601#Coordinated_Universal_Time_(UTC)>), такие временные координаты обозначаются буквой `Z` _(“Zulu time”)_.
Мы не убираем этот символ, чтобы не создать путаницы с местным временем или временем по Москве.

---

Разбивка папки `📂 web-pages` на подпапки помогает упорядочить архивы страниц, но не несёт в себе дополнительной информации.
Любая подпапка с файлом `🏛 web-page.json` считается папкой страницы.
Вложенных папок страниц не бывает:
...
1.  Клонируйте созданную для вас ветку в папку `/path/to/data/collections/COLLECTION_ID`.

    `COLLECTION_ID` — это название коллекции (например, `region-pnz` или `topic-xyz`).
    Название папки соответствует названию ветки репозитория с данными (`collections/COLLECTION_ID`).
...
1.  Откройте файл `/path/to/tooling/.env.local` как текстовый и укажите путь к коллекции архива.
    ...
    ```ini
    COLLECTION_DIR_PATH=/path/to/data/collections/COLLECTION_ID
    ```

    Части `/path/to` и `COLLECTION_ID` надо заменить.

<!--

1.  Если коллекция архива новая (то есть ещё не начата кем-то другим), заполните файл `/path/to/data/collections/COLLECTION_ID/collection-config.yml`.
...
Чтобы начать делать снимки, необходимо добавить в коллекцию архива первые несколько ссылок.
...
1.  Создайте файл `/path/to/data/collections/COLLECTION_ID/url-inbox.txt`.
...
Каждый «слайд» соответствует одному действию (например, прокрутка страницы вниз).

Программа Playwright имеет открытый исходных код и свободную лицензию.
...
1.  Обработайте очередь заявок на новые снимки:
    ...
    Успешные заявки по второму разу обрабатываться не будут.
...
1.  Проведите инвентаризацию существующих снимков Wayback Machine:
    ...
    Это могут быть как снимки, которые мы запрашивали ранее, так и снимки, которые делал кто-то другой.
...
Мы заранее не знаем, что из этого ценно, поэтому разделяем получение данных и их анализ.
...
1.  Извлеките сводки из снимков Playwright:
    ...
    Этот скрипт создаст файл `[web-page-dir]/snapshots/*-playwright.zip.summary.json` для каждого файла `[web-page-dir]/snapshots/*-playwright.zip`.

1.  Извлеките сводки из снимков Wayback Machine:
    ...
    Этот скрипт скачает известные нам снимки из `web.archive.org` и создаст файл `[web-page-dir]/snapshots/*-wayback-machine.zip.summary.json` для каждого файла `[web-page-dir]/snapshots/*-wayback-machine.zip`.

1.  Скомбинируйте сводки:
    ...
    Этот скрипт создаст файлы прочитает файлы `[web-page-dir]/snapshots/*.summary.json` и создаст файл `[web-page-dir]/snapshot-summary-combination.json`.
