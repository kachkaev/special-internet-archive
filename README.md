...
После запуска предыдущих скриптов у нас есть сводки по каждому снимку: `⏳ [web-page-path]/snapshots/*.summary.json`.
Информация в этих файлах повторяется, поэтому мы комбинируем сводки в файлы `⏳ [web-page-path]/snapshot-summary-combination.json`:
...
Имея снимки веб-страницы и сводки по ним, мы можем аннотировать веб-страницу: добавить метки, указать релевантные ссылки и так далее.
...
Координатор архива откроет веб-интерфейс командой `yarn dev`, увидит содержимое коллекции архива и сможет редактировать аннотацию каждой страницы.
...
Пользуясь уже собранными данными, мы можем расширять коллекцию архива без ручного труда.

1.  Запустите скрипт, который найдёт релевантные ссылки в собранных данных и добавит в файл `⏳ [collection-path]/url-inbox.txt`:

    ```sh
    yarn exe scripts/web-pages/auto-populate-url-inbox.script.ts
    ```

1.  ![][опционально]  
    Откройте файл `⏳ [collection-path]/url-inbox.txt` для проверки или ручного редактирования.

1.  Добавить в коллекцию ссылки из файла `⏳ [collection-path]/url-inbox.txt` , как это описано в разделе [Регистрация веб-страниц списком](#регистрация-веб-страниц-списком):
...
1.  Вернитесь в раздел [Создание и обновление снимков](#создание-и-обновление-снимков) и заново запустите все скрипты.
    Для только что добавленных веб-страниц нужны снимки, сводки снимков, комбинации сводок и аннотации.
